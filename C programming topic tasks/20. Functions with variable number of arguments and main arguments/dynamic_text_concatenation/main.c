...
char * concatenate(int count, ...)
{
    ...
    va_end(current_text);
    len += count;
    char* real_text = calloc(len, sizeof(char));
    if(real_text == NULL)
        return NULL;
    va_start(current_text, count);
    for(int i = 0; i < count; i++)
...
int main() {
    ...
    char* msg = concatenate(count_text, *(text_list+0), *(text_list+1), count_text >= 3 ? *(text_list+2) : NULL, count_text == 4 ? *(text_list+3) : NULL);
    if(msg == NULL)
    {
        for(int i = 0; i < 4; i++)
        {
